...
suite('NyarCode XLF Editor Extension Test Suite', () => {
	...
	test('Extension should activate', async () => {
		const extension = vscode.extensions.getExtension('nyarcode.nyarcode-xlf-editor');
		assert.ok(extension);
	...
	test('TreeView should be registered', () => {
		const treeView = vscode.window.createTreeView('nyarcode-xlf-editor-sidebar-view', {
			treeDataProvider: {
